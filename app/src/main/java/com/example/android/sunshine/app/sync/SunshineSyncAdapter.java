...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener {
    ...
    public static final int SYNC_INTERVAL = 30; // changed for quicker debugging
    ...
    private void getWeatherDataFromJson(String forecastJsonStr,
        ...
        try {
            ...
            for(int i = 0; i < weatherArray.length(); i++) {
                ...
                /*
                 * Send this data to the wearable
                 */
    ...
    private Asset createAsset(int resourceId) {
    ...
    private void notifyWearable(double highTemp, double lowTemp, int weatherAssetID) {
        Context context = getContext();

        mGoogleApiClient.connect();

        PutDataMapRequest putDataMapRequest = PutDataMapRequest.create("/weather-details");
        putDataMapRequest.getDataMap().putString("high_temp", Utility.formatTemperature(context, highTemp));
        putDataMapRequest.getDataMap().putString("low_temp", Utility.formatTemperature(context, lowTemp));

        Asset asset = createAsset(Utility.getIconResourceForWeatherCondition(weatherAssetID));
        putDataMapRequest.getDataMap().putAsset("weather_id", asset);

        /*
         * Add timestamp for testing- enable only for debugging
         */
